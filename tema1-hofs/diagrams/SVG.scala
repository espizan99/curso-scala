...
object SVG {
  ...
  def styleToSVG(ss: StyleSheet): String =
    ss.foldLeft("") {
      ...
      case (acc, FillColor(Maroon)) => acc + """fill="#aa0000" """
      case (acc, FillColor(LightGray)) => acc + """fill="#c0c0c0" """
      case (acc, FillColor(Purple)) => acc + """fill="#aa00aa" """
      case (acc, FillColor(NavyBlue)) => acc + """fill="#0000aa" """
      case (acc, FillColor(DarkGreen)) => acc + """fill="#00aa00" """
      case (acc, FillColor(Brown)) => acc + """fill="#aa5500" """
      case (acc, FillColor(Teal)) => acc + """fill="#00aaaa" """
      case (acc, _) => acc
    }
